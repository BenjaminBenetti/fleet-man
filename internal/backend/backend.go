...
type Backend interface {
	...
	// SupportsMicSink reports whether the backend can host the virtual
	// microphone's in-instance sink (see MicSinkCommand). A pure capability
	// answer — no I/O — so provisioning and teardown can ask it cheaply; when
	// false the instance simply has no virtual microphone and nothing is
	// installed for one.
	SupportsMicSink() bool
	...
	// microphone provider is attached. Returns (nil, false) exactly when
	// SupportsMicSink is false.

...
func newReadSessionCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Captures the contents of an existing tmux session and prints it to stdout.

By default only the currently visible pane is captured. Use --scrollback to
include lines that have scrolled off:
  -s 0   (default) visible pane only
  -s N   include the last N lines of scrollback before the visible pane
  -s -1  include the full scrollback history

This lets AI agents read what is currently on the screen inside a session
they created via 'spawn-session' and drove via 'exec-in-session'.

Examples:
  fleet read-session agent-1 my-session
  fleet read-session agent-1 my-session --scrollback 200
  fleet read-session my-fleet/agent-1 task-session -s -1`,
